...
const SoManagement = () => {
  ...
  function handleServiceOrderRegistration(e: FormEvent) {
    ...
    const so = {
      ...
      sellerId: seller,
      clientId: client,
      technicianId: responsibleTechnician,
    };

    api.post("/serviceOrders", so);

    api.post("/clients", { name: so.clientId });
    api.post("/sellers", { name: so.sellerId });
    api.post("/technicians", { name: so.technicianId });
  ...
  async function getSO() {
    try {
      const { data } = await api.get<SOProps[]>("/serviceOrders");
  ...
  async function closeOS(id: string) {
    try {
      const { data } = await api.patch(`/serviceOrders/${id}`, {
        running: false,
        completed: true,
        closed: false,
      });
  ...
  async function handleEndAnOSSubmit(e: FormEvent) {
    ...
    try {
      ...
      await api.patch(`/serviceOrders/${data[0].id}`, {
        manpower,
        displacement,
        running: false,
        completed: false,
